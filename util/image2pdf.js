 ...
 * @LastEditors: Bowen
 * @LastEditTime: 2022-03-06 17:20:09
 */
...
const logger = new Logger({ perfix: "[fuction/image2pdf]" });
...
/**
 * @description:
 * @param {*} dirname 需要转化的目录
 * @param {*} basePath 基础路径 同 dirname
 * @return {*} Stream 流
 */
async function PDFToZip(dirname, basePath, zip) {
  var files = await fs.readdir(dirname);
  for (const filename of files) {
    let filePath = path.resolve(dirname, filename);
    let stat = await fs.stat(filePath);
    // 是文件夹递归
    if (stat.isDirectory()) {
      let newDirname = path.resolve(dirname, filename);
      await PDFToZip(newDirname, basePath, zip);
      continue;
    }

    let file = await fs.readFile(filePath);
    // 去除basepath 和 /
    let zipFileName = filePath.replace(basePath, "").slice(1);
    await zip.file(zipFileName, file);
  }
  return zip;
}
